...
from alphapose_ros.msg import PointWithProb
...
def make_img(image_np):
    img = CompressedImage()
    img.header.stamp = rospy.Time.now()
    img.format = "png"
    img.data = np.array(cv2.imencode('.png', image_np)[1]).tostring() 
    image_pub.publish(img)

def bbox_writer(image_np,bbox,id_):
    cl = int(155 * id_)
    cv2.rectangle(image_np, (int(bbox[0]), int(bbox[1])), (int(bbox[0] + bbox[2]), int(bbox[1] + bbox[3])), (cl, 0, 0))

def id_writer(image_np,bbox,id_):
    cv2.putText(image_np,str(id_),(int((bbox[0] + bbox[2])/2.0), int((bbox[1] + bbox[3])/2.0)), cv2.FONT_HERSHEY_SIMPLEX, 2,(255,255,255),2,cv2.LINE_AA)

def point_writer(image_np,data):
    cv2.circle(image_np, (data[0], data[1]), 1, (255, 255, 255), thickness=-1)

def cb_pose(msg,data,image_np):
    alphapose = AlphaPoseHuman()
    for k in range(len(data["result"])):
        bbox = data["result"][k]["bbox"]
        id_ = data["result"][k]["idx"]
        pose_ = data["result"][k]["keypoints"]
        alphapose.body_bounding_box.x = bbox[0]
        alphapose.body_bounding_box.y = bbox[1]
        alphapose.body_bounding_box.width = bbox[2]
        alphapose.body_bounding_box.height = bbox[3]
        for pose_num in range(len(pose_)):
            point = pose_[pose_num]
            alphapose.body_key_points_with_prob[pose_num].x = float(point[0])
            alphapose.body_key_points_with_prob[pose_num].y = float(point[1])
            point_writer(image_np,point)            
        msg.human_list.append(alphapose)
        bbox_writer(image_np,bbox,id_)
        id_writer(image_np,bbox,id_)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        alphapose_list.header.stamp = rospy.Time.now()
        alphapose_list.header.frame_id = "camera"
        pose = demo.process(image)
        try:
            cb_pose(alphapose_list,pose,image)
        except:
            make_img(image)
        rt.sleep()
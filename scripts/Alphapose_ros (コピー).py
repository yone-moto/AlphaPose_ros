...
def make_img(image_np):
    ...
    img.format = "jpeg"
    img.data = np.array(cv2.imencode('.jpg', image_np)[1]).tostring() 
...
def id_writer(image_np,bbox,id_):
    ...
    cv2.putText(image_np,str(id_),(int(bbox[0]), int(bbox[1])), font, 2,(255,255,255),2,cv2.LINE_AA)

def point_writer(image_np,data):
    cv2.circle(image_np, (data[0], data[1]), 3, (255, 255, 255), thickness=-1)
...
if __name__ == '__main__':
    ...
    rt = rospy.Rate(20)
    subscriber = rospy.Subscriber("/image_raw/compressed",CompressedImage, cb_img,  queue_size = 1)
